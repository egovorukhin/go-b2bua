...
func NewUrlEncode(unrestricted []byte) *UrlEncode {
	...
	if unrestricted != nil {
		for _, c := range unrestricted {
			s.unrestricted[c] = true
		}
	}
	return s
}

func (s *UrlEncode) Escape(qry string) string {
...
func (s *UrlEncode) shouldEscape(c byte) bool {
	if (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z') || (c >= '0' && c <= '9') {
		return false
	}
	_, ok := s.unrestricted[c]
	return !ok
}

func (s *UrlEncode) Unescape(qry string) (string, error) {
	var err error
	inbuf := []byte(qry)
